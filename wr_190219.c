...
#define MAX_ETH    (32)
/** Максимальный адрес интерфейса. */
#define MAX_ETH_ADDR (31)
...
static unsigned int phy_addr = 0; /** Адрес PHY. */
static unsigned int reg_num = 0; /** Номер регистра. */
static unsigned int value = 0; /** Значение, которое будет записано в регистр. */
...
static int check_option_combination(bool flag_interface_set, bool flag_need_help,bool flag_write_to_register,bool flag_value_for_reg_set,bool flag_phy_addr_set,bool flag_register_number_set)
{
	...
	fprintf(stderr, "Incorrect arguments. -h for help.\n");
	//printf("%s \n", usage);
	return -1;
}

/** Проверка, введён ли адрес PHY правильно. */
static int check_addr(unsigned int phy_addr)
{
	if (phy_addr > 31){
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt_long(argc, argv, "i:wv:a:r:h", longopts, 0)) != EOF)
                switch (c) {
			case 'i': snprintf(s, optarg); flag_interface_set = true; break;
